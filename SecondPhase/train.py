...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Override radar beam count in observation (default: scenario setting).",
    )
    p.add_argument(
        "--radar-max-range-mm",
        type=float,
        default=None,
        help="Override radar max range in mm (default: scenario comm_r_mm).",
    )
...
def main() -> None:
    ...
    if args.radar_num_beams is not None:
        scenario_kwargs["radar_num_beams"] = int(args.radar_num_beams)
    if args.radar_max_range_mm is not None:
        scenario_kwargs["radar_max_range_mm"] = float(args.radar_max_range_mm)
    ...
    try:
        ...
        # Radar config (if enabled by the scenario).
        # 雷达配置（由 scenario 决定是否启用及其参数）。
        setattr(args, "radar_num_beams", int(getattr(scenario, "radar_num_beams", 0)))
        setattr(args, "radar_max_range_mm", float(getattr(scenario, "radar_max_range_mm", 0.0)))
    except Exception:
        pass
